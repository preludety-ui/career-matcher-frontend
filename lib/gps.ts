...
export async function construireGPS(
  ...
  // 3. Chercher les salaires réels Low/Median/High depuis salaires_cnp
  ...
  const annees_necessaires = Math.max(1, Math.min(5, ecart <= 0 ? 1 : Math.round(ecart * 1.5)))
  const objectif_atteignable = annees_necessaires <= 5

  // 5. Construire les 5 étapes avec salaires réels par ancienneté
  ...
  for (let annee = 1; annee <= 5; annee++) {
    ...
    etapes.push({
      annee,
      titre:       est_objectif ? metier_cible.titre_fr : metier_actuel.titre_fr,
