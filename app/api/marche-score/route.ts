...
export async function POST(req: NextRequest) {
    try {
        ...
        // Compter les vraies offres via JSearch
        let nbOffresReelles = 47;
        try {
            const jsearchUrl = `https://jsearch.p.rapidapi.com/search?query=${encodeURIComponent((objectif || '') + " " + (ville || "Montreal"))}&page=1&num_pages=1&country=ca`;
            ...
            if (jsearchData?.data?.length > 0) {
                nbOffresReelles = jsearchData.data.length * 12;
            }
        } catch (e) {
            console.error("JSearch marché error:", e);
        }
